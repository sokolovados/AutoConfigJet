...
region_checkboxes = db.Table('region_checkboxes',
                             ...
                             db.Column('city_id', db.Integer, db.ForeignKey('regions.id')))
region_input_checkboxes = db.Table('region_input_checkboxes',
                             db.Column('input_checkboxes_id', db.Integer, db.ForeignKey('input_checkboxes.id')),
                             db.Column('city_id', db.Integer, db.ForeignKey('regions.id')))

region_fields = db.Table('region_fields',
                             db.Column('fields_id', db.Integer, db.ForeignKey('fields.id')),
                             db.Column('city_id', db.Integer, db.ForeignKey('regions.id')))
...
class Regions(db.Model, UserMixin):
    ...
    inputcheckbox = db.relationship('InputCheckboxes', secondary=region_input_checkboxes,
                              backref=db.backref('input_checkboxes', lazy='dynamic'))
    ...
    fields = db.relationship('Fields', secondary=region_fields,
...
class Checkboxes(db.Model):
    ...
    checkbox_name = db.Column(db.String(80), unique=True, nullable=False)
    def __str__(self):
        return self.checkbox

class InputCheckboxes(db.Model):
    __tablenname__ = 'checkboxes'
    id = db.Column(db.Integer, primary_key=True)
    inputcheckbox = db.Column(db.String(80), unique=True, nullable=False)
    inputcheckbox_name = db.Column(db.String(80), unique=True, nullable=False)
    def __str__(self):
        return self.inputcheckbox
